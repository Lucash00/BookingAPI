		...
		http.csrf(csrf -> csrf.disable())
				.authorizeHttpRequests(auth -> auth
						.requestMatchers("/v3/api-docs/**", "/swagger-ui/**", "/swagger-ui.html").permitAll() // Swagger
																												// público
						.requestMatchers("/login", "/register").permitAll() // Login público
						...
						.requestMatchers("/api/bookings/**").hasAnyRole("USER", "ADMIN") // Usuarios y ADMIN pueden
																							// acceder a las reservas
						.anyRequest().permitAll() // El resto público
				)
				.addFilterBefore(new JwtAuthenticationFilter(jwtTokenUtils), UsernamePasswordAuthenticationFilter.class) // Ahora
																															// inyectamos
																															// JwtTokenUtils
